// lib/features/helpline/presentation/screens/helpline_screen.dart
...
class _HelplineScreenState extends State<HelplineScreen> {
  ...
  /// Opens Google Maps searching for nearby mental health services
  Future<void> _openMaps(BuildContext context) async {
    // Search query: mental health services near me — works globally
    final uri = Uri.parse(
      'https://www.google.com/maps/search/mental+health+services+near+me',
    );
  ...
  /// Opens the WHO mental health resources page in the browser
  Future<void> _openResources(BuildContext context) async {
    final uri = Uri.parse('https://www.who.int/health-topics/mental-health');
    if (!await launchUrl(uri, mode: LaunchMode.externalApplication)) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not open resources')),
        );
      }
    }
  }
